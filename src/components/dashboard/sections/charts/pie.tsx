...
export function PieChartComponent({ data }: { data: ChartData }) {
  ...
  data.data.forEach((element) => {
    element.data.forEach((statusElement) => {
      if (statusElement.status in statusCounts) {
        statusCounts[statusElement.status as keyof StatusCounts] +=
          statusElement.count;
      }
    });
  });
  ...
  const totalHadir =
    statusCounts.hadir + statusCounts.telat + statusCounts.dispen;

  const percentage = ((totalHadir / data.total) * 100).toFixed(2);
  ...
  return (
    <Card className="flex flex-col">
      <CardHeader className="items-center pb-0">
        ...
        <CardDescription>
          {djs().format("MMMM")} - {djs().format("YYYY")}
        </CardDescription>
      </CardHeader>
      ...
      <CardFooter className="flex-col gap-2 text-sm">
        ...
        <div className="leading-none text-muted-foreground">
          Total persentase data kehadiran berdasarkan bulan{" "}
          {djs().format("MMMM YYYY")}.
        </div>
      </CardFooter>
    </Card>
  );
